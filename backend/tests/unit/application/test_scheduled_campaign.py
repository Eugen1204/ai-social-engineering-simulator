...
@pytest.mark.asyncio
async def test_campaign_starts_when_time_came(employee_in_campaign):
    ...
    started = await service.execute(now=datetime_now)

    assert len(started) == 1
    assert started[0].id == campaign.id

    started_2 = await service.execute(datetime.now(timezone.utc))

    assert len(started_2) == 0


@pytest.mark.asyncio
async def test_campaign_start_with_the_same_time(application_template, application_organization):
    ...
    started = await service.execute(started)

    assert len(started) == 1


@pytest.mark.asyncio
async def test_campaign_not_started_before_scheduled_time(application_template, application_organization):
    repo_camp = CampaignRepoInMemory()
    template, _ = application_template
    org, _ = application_organization
    camp = Campaign(name=CampaignName("HR fishing2"),
...
@pytest.mark.asyncio
async def test_other_status_not_selected(application_template, application_organization):
    repo_camp = CampaignRepoInMemory()
    template, _ = application_template
    org, _ = application_organization
    camp = Campaign(name=CampaignName("HR fishing2"),
    ...
    started = await service.execute(datetime.now(timezone.utc) + timedelta(hours=2))
...
@pytest.mark.asyncio
async def test_with_3_campaigns_where_2_ready_to_started(application_template, application_organization,
    ...
    await repo_camp.save(camp_1)

    await repo_camp.save(camp_2)

    await repo_camp.save(camp_3)

    service = StartScheduledCampaign(repo_camp)

    result = await service.execute(now=datetime_now)
