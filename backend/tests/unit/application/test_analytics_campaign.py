...
async def test_risk_score_campaign(employee_in_campaign, application_organization):
    ...
    result_analytic = await service_analytic.execute(campaign_id=camp.id, organization_id=org.id)
...
@pytest.mark.asyncio
async def test_analytic_raises(employee_in_campaign, application_organization):
    org, repo_org = application_organization
    camp, repo_camp = employee_in_campaign
    ...
    with pytest.raises(CampaignResultsNotAvailableError):
        await service_analytic.execute(camp.id, org.id)

    camp.start()

    with pytest.raises(CampaignNotFoundError):
        await service_analytic.execute(uuid4(), org.id)

    with pytest.raises(OrganizationNotFoundError):
        await service_analytic.execute(camp.id, uuid4())
