...
class EngineTemplate:
    ...
    def _tokenize(self, template: Template) -> list[str]:
        tokens = []
        for match in self.token_pattern.findall(template.content):
            if match:
                tokens.append(match)
        return tokens
            ...
            if not token.startswith("{"):
                ...
                try:
                    ...
                    if condition_value:
                        ...
                        result.append(self._render_tokens(inner_tokens, context_template))
                except KeyError:
                    pass
                i = endif_index
    ...
    def render(self, template: Template, context_template: TemplateContext) -> str:

        tokens = self._tokenize(template)

        return self._render_tokens(tokens, context_template)
...
print(engine.render(Template(id=uuid4(), name="email",
                             content="Hello {{name}}", version=1), context_template=context))
