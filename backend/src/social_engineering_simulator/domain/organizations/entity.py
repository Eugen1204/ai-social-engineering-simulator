...
class Organization:
    ...
    def add_department(self, department: "Department") -> None:
        ...
        department.organization_id = self.id
        self._departments.append(department)

    def remove_department(self, department_id: UUID) -> None:
        department = self._find_by_id(department_id)
        if not department:
            raise DepartmentDontExistError(f"Department '{department}' dont exists in this organization")
        self._departments.remove(department)

    def _find_by_name(self, name: DepartmentName) -> "Department | None":
        return next((d for d in self._departments if d.name == name), None)

    def get_departments(self) -> tuple["Department", ...]:
        return tuple(self._departments)

    def get_department(self, dep_uuid: UUID) -> "Department | None":
